...
KUBECONFIG_NEW="$KUBECONFIG_DIR/config.lbrightlab"

echo "🔐 Setting up kubeconfig for lbrightlab local access..."
# Remove the old context if it exists
kubectl config delete-context lbrightlab >/dev/null 2>&1 || true
kubectl config delete-cluster lbrightlab >/dev/null 2>&1 || true
kubectl config delete-user lbrightlab >/dev/null 2>&1 || true
...
echo "🔄 Configuring context as 'lbrightlab'..."
...
# Rename context, user, and cluster to 'lbrightlab'
# Use global replace for context name (works with trailing space or no $); only this string appears as context name
sed -i.bak4 's/kubernetes-admin@kubernetes/lbrightlab/g' "$KUBECONFIG_NEW"
sed -i.bak5 's/name: kubernetes$/name: lbrightlab/' "$KUBECONFIG_NEW"
sed -i.bak6 's/cluster: kubernetes$/cluster: lbrightlab/' "$KUBECONFIG_NEW"
sed -i.bak7 's/user: kubernetes-admin$/user: lbrightlab/' "$KUBECONFIG_NEW"
sed -i.bak8 's/name: kubernetes-admin$/name: lbrightlab/' "$KUBECONFIG_NEW"
...
if [ -f "$KUBECONFIG_DEFAULT" ]; then
    ...
    # Remove existing lbrightlab context if it exists to ensure we use the new one
    # We do this on the DEFAULT file using a temporary KUBECONFIG env
    (
        export KUBECONFIG="$KUBECONFIG_DEFAULT"
        if kubectl config get-contexts lbrightlab >/dev/null 2>&1; then
             echo "   Removing existing 'lbrightlab' context to prevent conflicts..."
             kubectl config delete-context lbrightlab >/dev/null 2>&1 || true
             kubectl config delete-cluster lbrightlab >/dev/null 2>&1 || true
             kubectl config delete-user lbrightlab >/dev/null 2>&1 || true
        fi
...
echo ""
echo "🔄 Switching to context 'lbrightlab'..."
if kubectl config get-contexts lbrightlab &>/dev/null; then
    kubectl config use-context lbrightlab
    echo ""
    echo "🎉 Context 'lbrightlab' is ready and active!"
else
    ...
    if [ -n "$CONTEXT_FOR_6443" ]; then
        ...
        echo "   To rename it to 'lbrightlab': kubectl config rename-context '$CONTEXT_FOR_6443' lbrightlab"
    else
        echo "⚠️  Context 'lbrightlab' not found in merged config. Available contexts:"
